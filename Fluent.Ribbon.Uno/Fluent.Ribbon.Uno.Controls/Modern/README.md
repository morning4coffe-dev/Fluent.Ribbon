...
| Adaptive/visuals: touch density | P2 | ✅ done | Copilot | `Modern\Helpers\RibbonInputMode.cs`, `Themes\Modern\RibbonTouchDensity.xaml`, Showcase `MainPage.ModernShowcase.cs`, Showcase `MainPage.ModernAutoTest.cs`, `Modern\README.md` | `RibbonInputMode.SetInputMode` safely records mouse/touch preference in managed weak state. Apps merge `RibbonTouchDensity.xaml` before ribbon realization for template metrics; live dependency-property, dictionary, and template rewrites are intentionally avoided because WinUI cannot safely mutate realized ribbon density. |
